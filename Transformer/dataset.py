...
class GetDataset(object):
    ...
    def get_dataset(self, scale=True, stationary=False, indicators=False):
        ...
        '''
        x_df = self.df[["Close", "Open", "High", "Low", "Volume"]].dropna()[
            :-1]
        ...
        if stationary:
            ...
            y_df = analysis.get_stationary_data(self.df, ["Next_day_closing_price"], 12)[
    ...
    def split(self, train_split_ratio=0.8, time_period=30):
        ...
        x_train = [x_train_data[i-time_period:i]
        ...
        x_test = [x_test_data[i-time_period:i]
...
class GetDatasetOld(object):
    ...
    def split(self, train_split_ratio=0.8, time_period=30):
        ...
        self.y_train = np.array(y_train)
        self.x_train = np.reshape(np.array(x_train), (np.array(
            x_train).shape[0], np.array(x_train).shape[1], 1))
        print(
            f'Shape of train data: (x, y) = ({np.shape(self.x_train)}, {np.shape(self.y_train)})')
        ...
        self.x_test = np.reshape(np.array(x_test), (np.array(
            x_test).shape[0], np.array(x_test).shape[1], 1))
        print(
            f'Shape of test data: (x, y) = ({np.shape(self.x_test)}, {np.shape(self.y_test)})')
        return [self.x_train, self.y_train], [self.x_test, self.y_test], train_data_size
