...
def money_flow_index(df: pd.DataFrame, period: int):
    ...
    positive_mf = [sum(positive_flow[i + 1 - period:i + 1])
                   for i in range(period-1, len(positive_flow))]
    negative_mf = [sum(negative_flow[i + 1 - period:i + 1])
    ...
    for t_idx in range(len(df)):
        if t_idx + 1 <= period:
            ...
            mfi[t_idx] = 100 * (positive_mf[idx] /
                                (positive_mf[idx] + negative_mf[idx]))
            idx += 1
    df["MFI"] = mfi
    return df
...
def ulcer_index(df: pd.DataFrame, lookback: int):
    ...
    for t_idx in range(len(df)):
        if t_idx + 1 <= lookback:
            ...
            percentage_drawdown = [
                (df['Close'][t_idx-i]-maxprice)/maxprice * 100 for i in reversed(range(lookback))]
            ulcer_ind = np.sqrt(
                np.sum(np.array(percentage_drawdown)**2) / lookback)
            ui[t_idx] = ulcer_ind
    df['Ulcer_index'] = ui
    return df
...
def exponential_moving_average(df: pd.DataFrame, windows: list):
    for window in windows:
        df[f'EMA_{window}'] = df['Close'].ewm(
            span=window, adjust=False).mean().fillna(0)
    return df
