...
def positioning_encoding(seq_length, model_dim):
    ...
    frequencies = 1e-4 ** (2 * (torch.arange(model_dim,
                           dtype=torch.float) // 2) / model_dim).reshape(1, 1, -1)
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, n_layers=6, model_dim=512, output_dim=512,
        ...
        super().__init__()
        self.encoder = Encoder(n_layers, model_dim,
                               num_heads, forward_dim, dropout)
