...
def get_timestamps(yrs=0, mths=0, dys=0):
    ...
    start_time = datetime(end_time.year - yrs,
                          end_time.month - mths, end_time.day - dys)
...
def collect_data(timestamps, stock_name, moving_averages=None, include_gain=True, compute_volatility=False):
    ...
    locals()[stock_name] = DataReader(
        stock_name, 'yahoo', timestamps[0], timestamps[1])
...
def plot_gain(df):
    ...
    fig.add_trace(go.Histogram(x=df['Daily Return'].dropna(
    ), marker_color='#330C73', opacity=0.8), row=1, col=1)
    fig.add_trace(go.Scatter(x=xDR, y=df['Daily Return'].dropna(
    ), mode='lines', line_color='#330C73'), row=1, col=1)

    fig.add_trace(go.Histogram(x=df['Change %'].dropna(
    ), marker_color='#330C73', opacity=0.8), row=1, col=2)
...
def compare_stocks(dfs, timestamps):
    ...
    closing = DataReader(dfs, 'yahoo', timestamps[0], timestamps[1])[
        'Adj Close']
    daily_returns = closing.pct_change()
    x = [str(daily_returns.dropna().index[i]).split()[0]
